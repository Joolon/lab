 ...
 *
 * 正则中“字符”的含义
 * ^ 匹配开始字符  ：比如 ^foo 表示匹配以foo开头的字符串
 * $ 匹配结束字符  ：比如 $foo 表示匹配以foo结尾的字符串（^foo$ 表示以foo开头和结尾的字符串，只能是foo）
 ...
 * [xyz]  匹配一个集合中任何一个字符（匹配一个字符，匹配一次） ：[xyz] 匹配 x、y、z
 * [^xyz] 匹配一个集合之外的任何字符 ：[^xyz] 匹配非x、y、z
 * [a-z]  匹配 a-z
 * [^a-z] 匹配非 a-z中的字符
 *
 * \b  匹配一个单词的边界
