...
// 如何运行
// 要先运行该文件，CMD命令运行，如果关闭CMD窗口则客户端会退出聊天室   \> php Sock.php
// 多个标签同时打开网页  http://www.lab.net/Demo/WebSocket/chat/chat.html  发送数据后都可以看到消息，实现聊天功能
// Sock.php 是一个服务器，把数据分发到用户中
...
class Sock{
    ...
    function run(){
        ...
        while(true){
			...
			if(!empty($this->users)){// 调试信息
				//file_put_contents('D:\www\lab\Demo\WebSocket\chat\log.txt',json_encode($this->users));
			}
