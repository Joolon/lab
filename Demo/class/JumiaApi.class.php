...
//            'CreatedBefore'=>'2017-09-22',
    ...
    function getorders($order_result,$url,$apikey){
        ...
        foreach ($order_array as $key1 => $value111){
            if($this->isjumiaorder($value111['OrderNumber'],$url)){// 判断基础表中订单是否存在，存在则跳过
                continue;
            }
            //插入到jumia_order表
            $OrderId = $value111['OrderId'];
            $CustomerFirstName = mysql_real_escape_string($value111['CustomerFirstName']);
            ...
            $AddressBilling_FirstName = mysql_real_escape_string($value111['AddressBilling']['FirstName']);
            $AddressBilling_LastName = mysql_real_escape_string($value111['AddressBilling']['LastName']);
            ...
            $addtime = time();
            $jumia_sql="INSERT INTO `jumia_order`(`OrderId`,`CustomerFirstName` ,`CustomerLastName`,`OrderNumber` ,`PaymentMethod` ,`Remarks` ,`DeliveryInfo` ,
                    ...
                    `AddressShipping_Address2`,`AddressShipping_CustomerEmail`,`AddressShipping_City`,`AddressShipping_Ward`,`AddressShipping_Region`,`AddressShipping_PostCode`,`AddressShipping_Country`,`addtime`,`zhandian`)
                    VALUES ('$OrderId','$CustomerFirstName', '$CustomerLastName' , '$OrderNumber' , '$PaymentMethod' , '$Remarks' , '$DeliveryInfo' ,
                     ...
                     '$AddressShipping_Address2','$AddressShipping_CustomerEmail','$AddressShipping_City','$AddressShipping_Ward','$AddressShipping_Region','$AddressShipping_PostCode','$AddressShipping_Country','$addtime','$url')";
            if(self::$dbcon->execute($jumia_sql)){
                echo '--jumnia_order success--';
            }else{
                echo '--jumnia_order failure--';
                echo $jumia_sql;
            }
            if($this->judgeOrderExists($value111['OrderNumber'],'jumia-fangfang118@yahoo.com')){// 判断订单是否存在，存在则跳过
                continue;
            }
    ...
    function saveOrder($order,$url,$apikey){
        ...
        //订单信息
        $ordersn = 'jumia-fangfang118@yahoo.com-'.$order['OrderNumber'];// V2系统订单编号（账户+订单ID）
        ...
        $ebay_account = 'jumia-fangfang118@yahoo.com';
        ...
        $order_sql = "INSERT INTO `ebay_order`(`ebay_paystatus`,`ebay_ordersn` ,`ebay_orderid`,`ebay_createdtime` ,
                    ...
                    VALUES ('$ebay_paystatus','$ordersn', '$recordnumber' , '$createdtime' ,
                     ...
                     '$paystatus','$defaultStoreId','$order_no','JUMIA','$status')";
        ...
        if(is_array($orderitem)){
            ...
            foreach ($orderitem_new as $newkey => $newvalue){
                ...
                //将数据插入jumia_order_detail表
                $jumia_sql = "INSERT INTO `jumia_order_detail` (`OrderItemId`,`orderdetail_item_id` ,`ShopId` ,`OrderId` ,`Name`,`Sku`,`ShopSku`,
                        `ItemPrice` ,`PaidPrice`  ,`WalletCredits`,`TaxAmount`,`ShippingAmount`,`VoucherAmount`,`IsDigital`,
                        `TrackingCode`,`PurchaseOrderId`,`PurchaseOrderNumber`,`PromisedShippingTimes`,`ExtraAttributes`,`CreatedAt`,`UpdatedAt`,`ShipmentProvider`)
                        VALUES ('$OrderItemId','$orderdetail_item_id', '$ShopId' , '$OrderId' , '$Name' ,'$Sku' ,'$ShopSku' ,
                        '$ItemPrice' ,'$PaidPrice'  ,'$WalletCredits' ,'$TaxAmount' ,'$ShippingAmount' ,'$VoucherAmount' ,'$IsDigital',
                         '$TrackingCode','$PurchaseOrderId','$PurchaseOrderNumber','$PromisedShippingTimes','$ExtraAttributes','$CreatedAt','$UpdatedAt','$UpdatedAt')";
                if(self::$dbcon->execute($jumia_sql)){
                    echo '--jumia_detail success--';
                }else{
                    echo '--jumia_detail failure--';
                    echo $jumia_sql;
                }
            }
            //计算产品数量,同时根据sku去重复(将sku写入键)
            ...
            foreach ($new_order as $key =>$value ){
                ...
                $itemid= $value["OrderId"].'-'.$sku;// 订单明细编号(存储OrderId和SKU关联jumia_order_detail表orderdetail_item_id字段,用于交运)
     ...
     * @return bool
     */
    function isjumiaorder($order_id,$zhandian){
        $sql	= "select id from jumia_order where OrderNumber='{$order_id}' and zhandian ='$zhandian' limit 1";
...
//                'OrderItemId'=>$itemlist[0]['OrderItemId']
    ...
    function updatetoshipptime(){
        $sql = "SELECT ebay_id,ebay_ordertype,ebay_paidtime,ebay_status from ebay_order WHERE ebay_ordertype = 'Jumia' AND ebay_status in (274,1)";
        $sql	= self::$dbcon->execute($sql);
