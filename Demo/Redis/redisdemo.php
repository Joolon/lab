...
// Redis是什么？ Redis是一个完全开源免费的（C语言编写），遵循BSD协议的，高性能的Key-Value数据库。redis是一种典型的no-sql 即非关系数据库。
...
// 雪崩：由于Redis服务器重启或不可用导致大量应该访问缓存层的请求直接访问了存储层，导致DB的调用量猛增，服务器甚至宕机。
// 解决办法：1、Redis服务器集群、DB服务器集群保证可用性；2、Redis锁机制只允许一个进程更新数据库Key。


/*

分布式网络：
	集群：通过增加服务器的数量，提供相同的服务，相同的功能，从而提供一个稳定、高效的服务状态，防止一台服务器宕机导致整个服务不可用。
	为什么使用集群？1、单台Redis服务器不稳定；2、单台Redis服务器处理事务能力有效；


	Redis 集群：多台 Redis 服务器，每台 Redis 服务器是一个节点，其中有一个主节点和多个子节点。

	Redis Cluster 是redis官方提供的分布式解决方案，在3.0版本后推出的，有效地解决了redis分布式的需求，
	当一个redis节点挂了可以快速的切换到另一个节点。
	当遇到单机内存、并发等瓶颈时，可以采用分布式方案要解决问题。


	主从复制：一个主从网络中只有一个主节点Master，多个从节点Slave。只有Master可读可写，Slave只能读不能更新，
	所以提高了读的能力，从而释放了Master读的能力，变相的提高了主从模型写的能力。
